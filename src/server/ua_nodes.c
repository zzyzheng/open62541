...
UA_Node_deleteReferencesSubset(UA_Node *node, size_t referencesSkipSize,
    ...
    for(size_t i = 0; i < node->referencesSize; ++i) {
        ...
        if(i == node->referencesSize) /* Last element? referencesSize was already reduced by 1. */
            continue;
        node->references[i] = node->references[node->referencesSize];
        i--; /* Repeat at the current position */
    }
